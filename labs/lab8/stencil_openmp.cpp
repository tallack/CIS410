...
void stencil(const int rows, const int cols, pixel * const in, pixel * const out){

	const int rad=1;
	const int dim= rad*2+1;
		...
		for(int j=0; j<cols; ++j){
			...
			for(int x=i-rad, kx=0; x<=i+rad; ++x, ++kx){
				for(int y=j-rad, ky=0; y<=j+rad; ++y, ++ky){
					...
					pixelX+=kernelx[k_off]*(in[in_off].red+in[in_off].green+in[in_off].blue)/3;
					pixelY+=kernely[k_off]*(in[in_off].red+in[in_off].green+in[in_off].blue)/3;
